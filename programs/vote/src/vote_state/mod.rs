...
pub fn update_commission<S: std::hash::BuildHasher>(
    ...
    clock: &Clock,
    allow_commission_decrease_at_any_time: bool,
    commission_updates_only_allowed_in_first_half_of_epoch: bool,
) -> Result<(), InstructionError> {
    // Decode vote state only once, and only if needed
    let mut vote_state = None;

    let enforce_commission_update_rule = if allow_commission_decrease_at_any_time {
    ...
    if enforce_commission_update_rule && commission_updates_only_allowed_in_first_half_of_epoch {
